...
export function stringifyRule(rule: Rule): string {
	return `Rule[name: ${rule.name}, description: ${rule.description}, categories: ${rule.categories.join(',')}, languages: ${rule.languages.join(',')}, engine: ${rule.engine}, sourcepackage: ${rule.sourcepackage}]`;
}
